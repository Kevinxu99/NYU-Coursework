...
def restore_bst_helper(prefix_lst, ind, curr_min, curr_max):
    if (ind >= len(prefix_lst)):
        return None
    if prefix_lst[ind]<curr_max and prefix_lst[ind]>curr_min:
        item = BinarySearchTreeMap.Item(prefix_lst[ind])
        curr_root = BinarySearchTreeMap.Node(item)
        curr_root.left = restore_bst_helper(prefix_lst, ind+1, curr_min, prefix_lst[ind])
        curr_root.right = restore_bst_helper(prefix_lst, ind + 1, prefix_lst[ind], curr_max)
    else:
        curr_root = restore_bst_helper(prefix_lst, ind+1, curr_min, curr_max)
    return curr_root

def restore_bst(prefix_lst):
    bst = BinarySearchTreeMap()
    bst.root = restore_bst_helper(prefix_lst, 0, -float('inf'), float('inf'))
    return bst

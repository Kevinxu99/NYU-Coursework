...
class LinkedBinaryTree:
    ...
    def find_leaves(subtree_root):
        if(subtree_root.left == None and subtree_root.right == None):
            yield subtree_root.data
        elif(subtree_root.left == None):
            for curr in LinkedBinaryTree.find_leaves(subtree_root.right):
                yield curr
        elif(subtree_root.right == None):
            for curr in LinkedBinaryTree.find_leaves(subtree_root.left):
                yield curr
        else:
            for curr in LinkedBinaryTree.find_leaves(subtree_root.left):
                yield curr
    ...
    def leaves_list(self):
        ...
        if(self.root == None):
            return leaf_lst
        for curr in LinkedBinaryTree.find_leaves(self.root):
            leaf_lst.append(curr)
        return leaf_lst
